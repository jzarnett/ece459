...
\item Finally, look around on the Internet for crates that implement concurrent hash maps, and use a single concurrent hash map for \texttt{dbl} and one for \texttt{trpl}. Evaluate the performance of that---we are expecting it to be faster than the original. Use this implementation when \texttt{--single-map} is not specified.
\end{enumerate}
