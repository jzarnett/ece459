...
Massif has found all the allocations in this program and distilled them down to a a tree structure that traces the path through which all of these various memory allocations occurred. So not just where the \texttt{malloc} call happened, but also how we got there.
