...
We're going to focus on low-level I/O from sockets in this lecture
...
let events = Events::with_capacity(128);
    \end{lstlisting}
We're going to create \texttt{events} right now; it is used by
...
\begin{itemize}[noitemsep]
\item You're telling it to check for when 
when the \texttt{listener} indicates that something is available to read,
indicated with \texttt{READABLE}.
\item The \texttt{SERVER} parameter
is a note to itself saying that events indicated with this particular
...
As foreshadowed, \texttt{poll.poll} is to populate \texttt{events}, and
