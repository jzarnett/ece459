...
println!("body = {:?}", body);
\end{lstlisting}
(The documentation points out that if you are doing multiple requests,
you should create your own \texttt{Client} and re-use it instead of
calling \texttt{reqwest::get}).
...
this. Otherwise I'll briefly explain futures from first principles.
...
usual, they are actually out of bicycles at the moment. So they can't
...
one depends on your context. Rust therefore allows you to specify a runtime
which defines the meaning of \texttt{async/await} for your program.

The simplest \texttt{await} just blocks and waits for the result to be ready.
Rust provides \texttt{futures::executor::block\_on} to provide that
...
There are other tags to choose other executors.
\begin{lstlisting}[language=Rust]
#[tokio::main]
async fn main() {
    // do async stuff
