...
There are still the global initialization and cleanup functions. The structure for the new multi-handle type is \texttt{curl::multi::Multi} (instead of \texttt{curl::easy::Easy}) and it is initialized with the \texttt{new()} function. 

Once we have a multi handle, we can add easy handles---however many we need---to the multi handle. Creation of the easy handle is the same as it is when being used alone---use \texttt{Easy::new()} to create it and set options on that handle. The documentation suggests that an \texttt{Easy2} might be better for use with multi handles. We'll want to set a token on this easy handle so that we can look it up later. Then, we add the easy (or easy2) handle to the multi handle with \texttt{add()} (or \texttt{add2()}).
...
The first parameter is an array of extra file descriptors you can wait on (but we will always want this to be \texttt{\&mut []} in this course). The second parameter is a maximum time to wait. The return value is the actual number of ``interesting'' events that occurred (interesting is the word used in the specifications, and what it means is mysterious). For a simple use case you can ignore most of the parameters and just wait for something to happen and go from there.
...
Why both? Well---one is about what the status of the request is. The message could be, for example, ``done'', but does that mean finished with success or finished with an error? For the second one tells us about that. We can ask about the status of the request using \texttt{messages()}.
...
When a handle has finished, you need to remove it from the multi handle. You can look it up using the \texttt{token} that you associated with the easy handle when you created it. Remove it with \texttt{remove} or \texttt{remove2}. You don't have to cleanup the easy handle because Rust.

Let's consider the following code example by Clemens Gruber~\cite{curlmulti}, with slight modifications for compactness and formatting. This example puts together all the things we talked about in one compact code segment. Here, the callback does nothing, but that's okay---it's just to show what you could do with it.
