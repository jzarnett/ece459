...
A hybrid between ``do less work'' and ``be smarter'' is a, where
...
to improve things dramatically by going to solid-state drives; or you might be swapping
out to disk, which kills performance (add RAM). Profiling is key here, to find out what the slow parts of execution are. When it comes down to it, spending a few thousand dollars on better hardware is often much cheaper than paying programmers to spend their time to optimize the code.
