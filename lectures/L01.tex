...
\paragraph{Improve the hardware.} Once upon a time, it was okay to write code with terrible performance on the theory that next year's CPUs would make it acceptable, and spending a ton of time optimizing your code to run on today's processors was a waste of time. Well, those days seem to be over; CPUs are not getting much faster these days (evolutionary rather than revolutionary changes). But sometimes the CPU is not the limiting factor: your code might be I/O-bound, so you might be able
...
multiprocessor) systems, or a cluster of machines. We get different
...
First, some domains are ``embarrassingly parallel'' and these problems
...
states never become visible (typically done using locks). 
