...
sys	0m0.000s
\end{lstlisting}
Some versions of {\tt gcc} seem to have gotten smart enough to reject bogus hints since then. In 2018, gcc 7.2.0 does show a difference of 2.12s (correct guess) vs 2.47s (wrong guess, no guess) on Linux 4.14.12 (post-mitigation) running on a Pentium G645.
...
\[
\mathrm{branch\_\%} \times 1 \mathrm{~cycle} + \mathrm{non\_branch\_\%} \times 20 \mathrm{~cycles} = 4.8 \mathrm{~cycles}.
\]
With perfect branch prediction:
\[
\mathrm{branch\_\%} \times 1 \mathrm{~cycle} + \mathrm{non\_branch\_\%} \times 1 \mathrm{~cycle} = 1 \mathrm{~cycle}.
\]
