extern "C" __constant__ float EPSILON =  1e-10;
...
void body_body_interaction(float4 point1, float4 point2, float4 *acceleration) {
    float4 difference;
    ...
    float distSqr = difference.x * difference.x + difference.y * difference.y + difference.z * difference.z + EPSILON;
