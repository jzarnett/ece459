...
\begin{frame}
	...
	But all of these are analysis tools for your programs.

	They slow down performance, but they are wonderful for finding errors in your program.
...
\begin{frame}
	...
	The purpose of memcheck is to look into all memory reads, writes, and to intercept and analyze every call to allocate/free memory.
\end{frame}
...
\begin{frame}
\frametitle{Memory Leaks? In My Rust?}

\begin{center}
	\includegraphics[width=0.4\textwidth]{images/thanos.jpg}
\end{center}

Here are some ways that you can end up with memory leaks in Rust:

\begin{itemize}
	\item Explicitly calling the \texttt{leak} and \texttt{forget} functions (which have corner case uses)
...
\begin{frame}
	...
	It will report as an error the initial order (the first order seen is ``correct'') and the the ``incorrect'' order that is the source of the potential problem.
